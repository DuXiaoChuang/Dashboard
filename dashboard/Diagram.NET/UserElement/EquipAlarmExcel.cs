...
namespace Dalssoft.DiagramNet
{
    ...
    public class EquipAlarmExcel : BaseElement, IControllable
    {
        [NonSerialized]
        ...
        protected int line1 = 125;
        protected int line2 = 125;
        protected int line3 = 125;
        ...
        public EquipAlarmExcel()
            : this(0, 0, 100, 100)
        { }

        public EquipAlarmExcel(Rectangle rec)
            : this(rec.Location, rec.Size)
        { elementMonitoredType = MonitoredType.设备报警信息表; }

        public EquipAlarmExcel(Point l, Size s)
            : this(l.X, l.Y, s.Width, s.Height)
        { }

        public EquipAlarmExcel(int top, int left, int width, int height)
        {
        ...
        internal override void Draw(Graphics g)
        {
            ...
            g.DrawString("设备编号", headfont, new SolidBrush(stringheadColor), r1, sf);
            g.DrawString("设备名称", headfont, new SolidBrush(stringheadColor), r2, sf);
            g.DrawString("报警内容", headfont, new SolidBrush(stringheadColor), r3, sf);
            g.DrawString("报警开始时间", headfont, new SolidBrush(stringheadColor), r4, sf);
            ...
            if (MoniteredObjectID != "")
            {
                if (statisticstyle1.ToString() == "无")
                    sql = "select EquipCode, EquipName, AddressContent, StateStarttime from Equip_State_P  where   reserve5='" + monitoredObjectID + "'and StateDescription = '设备报警'order by StateStarttime";
                else if (statisticstyle1.ToString() == "区域1")
                    sql = "select EquipCode, EquipName, AddressContent, StateStarttime from Equip_State_P  where    reserve5='" + monitoredObjectID + "'and StateDescription = '设备报警'and (equipcode in (select equipcode from EquipMent_B where zone=1))order by StateStarttime ";
                else if (statisticstyle1.ToString() == "区域2")
                    sql = "select EquipCode, EquipName, AddressContent, StateStarttime from Equip_State_P  where    reserve5='" + monitoredObjectID + "'and StateDescription = '设备报警'and (equipcode in (select equipcode from EquipMent_B where zone=2))order by StateStarttime ";
                else if (statisticstyle1.ToString() == "区域3")
                    sql = "select EquipCode, EquipName, AddressContent, StateStarttime from Equip_State_P  where    reserve5='" + monitoredObjectID + "'and StateDescription = '设备报警'and (equipcode in (select equipcode from EquipMent_B where zone=3))order by StateStarttime ";
            ...
            {
                if (statisticstyle1.ToString() == "无")
                    sql = "select EquipCode, EquipName, AddressContent, StateStarttime from Equip_State_P where StateDescription = '设备报警' order by StateStarttime";
                else if (statisticstyle1.ToString() == "区域1")
                    sql = "select EquipCode, EquipName, AddressContent, StateStarttime from Equip_State_P  where StateDescription = '设备报警'and (equipcode in (select equipcode from EquipMent_B where zone=1))order by StateStarttime ";
                else if (statisticstyle1.ToString() == "区域2")
                    sql = "select EquipCode, EquipName, AddressContent, StateStarttime from Equip_State_P  where StateDescription = '设备报警'and (equipcode in (select equipcode from EquipMent_B where zone=2))order by StateStarttime ";
                else if (statisticstyle1.ToString() == "区域2")
                    sql = "select EquipCode, EquipName, AddressContent, StateStarttime from Equip_State_P  where StateDescription = '设备报警'and (equipcode in (select equipcode from EquipMent_B where zone=3))order by StateStarttime ";
            }
            DataTable dt = DbHelperSQL.OpenTable(sql);
            if (dt.Rows.Count > 0)
            {
